...
namespace DBMS_Nhom3.BSLayer
{
    class BLTransaction
    {
        ...
        {
            string sqlString = "Update Transactions Set Total_price = " + totalprice.ToString() + ", Date = CONVERT(datetime,'" + date + "',103)" + ", ID_customer = " + idcus.ToString() + ",username = '" + username + "' where ID_transaction = " + idtrans.ToString();
            return db.MyExecuteNonQuery(sqlString, CommandType.Text, ref err);
        }
    }
}
